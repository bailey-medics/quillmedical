...
def _make_teaching_org(db: Session) -> OrgUnit:
...
def _make_educator(db: Session, org: OrgUnit) -> User:
...
def _make_learner(db: Session, org: OrgUnit) -> User:
...
class TestPromotingAVersion:
    ...
    def _status(self, db_session, org):
        return (
            ...
            .filter_by(org_unit_id=org.id, question_bank_id="test-bank")
            .one()
        )
    ...
    def test_another_organisations_version_is_not_promotable(
        ...
        educator = _make_educator(db_session, org)
        _seed_bank(db_session, org.id, educator.id)
        other = OrgUnit(name="Other Org", type="hospital_team")
    ...
    def test_promoting_for_an_organisation_you_are_not_in_is_refused(
        ...
        educator = _make_educator(db_session, org)
        self._with_two_versions(db_session, org, educator)
        other = OrgUnit(name="Not Mine", type="hospital_team")
        db_session.add(other)
        db_session.commit()
...
class TestBankOrgSettingsAreScopedToYourOrganisations:
    ...
    def test_a_bank_held_only_by_your_second_organisation_is_found(
        ...
        second = OrgUnit(name="Second Org", type="hospital_team")
        db_session.add(second)
        db_session.flush()
        add_org_unit_member(db_session, second.id, educator.id, "trainee")
        db_session.commit()
        ...
        row = (
            db_session.query(QuestionBankOrgStatus)
            .filter_by(org_unit_id=second.id, question_bank_id="test-bank")
            .one()
        )
...
class TestLearningContentGate:
    ...
    def test_helper_takes_any_live_org_when_the_user_has_two(self, db_session):
        ...
        add_org_unit_member(db_session, with_live.id, learner.id, "trainee")
        db_session.commit()
    ...
    def test_a_site_member_reaches_the_organisation(
        ...
        learner = _make_learner(db_session, org)
        remove_org_unit_memberships(db_session, learner.id)
...
class TestMediaCaptions:
    ...
    **The bucket-backed success path is not reachable here.** The test
    ...
    def test_reading_an_asset_that_is_not_ours_is_not_found(
        ...
        org = _make_teaching_org(db_session)
        _make_educator(db_session, org)
        other = OrgUnit(name="Another Trust", type="hospital_team")
        db_session.add(other)
        db_session.flush()
