...
JWT secret reuse for CSRF tokens
...
CSRF token signing uses same JWT_SECRET as JWT tokens, increasing attack surface where single secret compromise breaks both authentication and CSRF protection mechanisms.

---

## Causes

1. make_csrf and verify_csrf functions use settings.JWT_SECRET for signing
2. Same secret used for two different security mechanisms
3. If JWT secret compromised (logs, config dump, environment variable exposure), both protections fail
