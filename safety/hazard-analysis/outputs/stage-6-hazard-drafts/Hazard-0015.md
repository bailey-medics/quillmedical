...
EHRbase EHR creation race condition
...
get_or_create_ehr function has check-then-create race condition where two concurrent requests can both create EHR for same patient because operation is not atomic.

---

## Causes

1. Function checks for existing EHR then creates if missing (not atomic)
2. No database-level unique constraint on subject_id
3. Two FastAPI workers handle requests simultaneously for same patient
