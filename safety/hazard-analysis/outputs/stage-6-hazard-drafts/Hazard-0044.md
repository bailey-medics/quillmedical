...
If Caddyfile configures CORS headers with wildcard Access-Control-Allow-Origin (\*), malicious websites can make authenticated API requests using victim's cookies.

---

## Causes

1. Access-Control-Allow-Origin set to wildcard "\*"
2. No credentials=true validation
3. Allows malicious site to make cross-origin requests with authentication
