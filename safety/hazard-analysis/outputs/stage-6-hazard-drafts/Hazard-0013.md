...
Gender input converted to lowercase without proper validation against FHIR R4 required values (male|female|other|unknown), potentially allowing invalid values to be stored.

---

## Causes

1. Input converted to .lower() but typos like "femail" pass lowercase conversion
2. No validation against FHIR R4 enum before submission
3. FHIR server may reject or silently store invalid value
