...
Push notifications lack user association
...
Push subscriptions stored in-memory list without user_id foreign key, preventing selective notification targeting and causing all notifications to be sent to all subscribed devices indiscriminately.

---

## Causes

1. In-memory subscription list stores subscriptions without user context
2. No database table with user_id foreign key
3. Backend sends to all subscribers without filtering by role or patient assignment
