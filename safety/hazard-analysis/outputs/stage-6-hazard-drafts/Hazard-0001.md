...
Patient demographics (name, DOB, NHS number) displayed in UI do not match the actual patient record being viewed due to React rendering race conditions, state management bugs, or component key collisions.

---

## Causes

1. Race condition where patient prop updates but stale data renders before React reconciliation
2. Parent component passes wrong patient object due to state management bug
3. React key collision causing incorrect component instance to display
4. React closure captures patient.id from props at render time but list re-sorts/filters between mouse-down and mouse-up
5. Patient state stored in component state rather than React Router loader, lost on navigation
