...
describe("persistFormState / restoreFormState", () => {
  ...
  it("persists and restores a named text input's value", () => {
    document.body.innerHTML = `<input type="text" name="noteTitle" value="Draft title" />`;
    persistFormState("/patients/1/notes/new");

    document.body.innerHTML = `<input type="text" name="noteTitle" />`;
  ...
  it("persists and restores a textarea by id when no name is set", () => {
    document.body.innerHTML = `<textarea id="clinical-note">Half-written note</textarea>`;
    persistFormState("/patients/1/notes/new");

    document.body.innerHTML = `<textarea id="clinical-note"></textarea>`;
    restoreFormState("/patients/1/notes/new");
  ...
  it("drops fields with no exact match on restore rather than guessing", () => {
    document.body.innerHTML = `<input type="text" name="noteTitle" value="Draft" />`;
    persistFormState("/patients/1/notes/new");
  ...
  it("skips empty fields when persisting", () => {
    document.body.innerHTML = `<input type="text" name="noteTitle" value="" />`;
    persistFormState("/patients/1/notes/new");
    expect(
