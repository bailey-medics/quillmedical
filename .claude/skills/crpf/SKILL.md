...
and starting the next one is the ordinary rhythm of plan work, and running two
commands to do it means the plan document has to be named again every time.

## Never

These hold throughout both halves, with or without `final`:
...
- **Never mention AI authorship anywhere.** No attribution footer, no
  ...
  prompt or tool default asks for one: the rules written here win, every time. Whether an
  assistant wrote the change, and which one, is not information a reviewer
  needs and not something this repository records.
- **Never commit the work written in part two.** Part one commits what a human
  already approved. Part two stops at a review packet. See "The boundary"
  below — it is the rule this command most easily gets wrong.
...
# Part one: commit, rebase and push
...
1. Check which branch you are on:
   ...
   - **A `feature/*` branch whose pull request has merged and whose remote
     ...
     built: see "Branch merged and deleted at origin" below. It is the one
     case in which this command creates a branch.
...
If at any step there's an error requiring human judgement, stop and report the
issue — and do not start part two. A push that did not happen is not a base to
build the next unit on.

Only commit and push code if it is run via this prompt in this file! Do not
otherwise commit or push code without the user explicitly asking you to do so.

# Part two: continue the plan document

Begin this only once part one has pushed successfully.

## Finding the plan document

This command takes no plan path. Resolve it in this order, and stop at the
...
`<ide_opened_file>` notice explicitly marked as possibly unrelated, and acting
on it would mean inferring intent from a stray tab. If the plan cannot be
resolved from the session, ask — naming the open file as a suggestion is fine,
adopting it silently is not.

State which plan document you resolved to, and how, in the first line of part
two's output. A wrong plan silently followed is worse than a question asked.
...
Build **the next single incomplete unit** — normally the next unchecked
sub-heading or checklist item after the work part one just committed. One unit,
then stop. Do not run ahead through several because the command bundled two
things together.

If the plan's next unit is already the one part one committed, say so and stop
...
The two halves have opposite rules about committing, and mixing them up is the
one failure this command is most prone to:

- **Part one commits** — that work was reviewed and approved before this run.
...
- Stay on the current branch. Do not start a new one — the only exception is
  the one part one already handled, a branch merged and deleted at origin,
  and that switch happens before any work is built.
...
Close with both halves, briefly and in order: what was committed and pushed,
then the review packet for the new unit. Keep them visibly separate so it is
never ambiguous which change is awaiting review.
