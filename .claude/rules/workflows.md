...
- **Run the suite with `just test-scripts` (alias `ts`).** It runs that same script inside `ubuntu:24.04` — the runner image, the pinned bats version, and both targets (`.github/scripts` and `.claude/hooks`), so a local pass means what a CI pass means. It needs Docker and installs no bats on the host.
- **Do not judge a script by a bare `bats` run on macOS.** The scripts target ubuntu runners; the host has a different bash (3.2 by default, so no `${var^}` or other bash 4 syntax), a different coreutils, and a `sha256sum` that is not GNU's. That gap has produced both a failure only the host could see and a pass that proved nothing. Use it to iterate if you like, but the container decides.
