...
4. Runs every test marked `migration` (`pytest -m migration`). Each is
   ...
   The rest step the database back to the revision before a data
   migration, seed it, and step forward to check what the migration wrote.
   A new test of this kind is picked up by carrying the marker, with no
