...
Missing CSRF protection on clinical operations
...
POST /api/patients/{patient_id}/letters and other state-changing clinical operations do not require CSRF token, vulnerable to Cross-Site Request Forgery attacks.

---

## Causes

1. Route uses `require_roles("Clinician")` but not `require_csrf` dependency
2. CSRF token only required on some routes inconsistently
3. Malicious website can forge requests using victim's session cookies
