...
Letters list page displays letters from EHRbase AQL query but does not handle pagination limits or auto-refresh, potentially missing recent letters if query has default row limit or letters created during page load.

---

## Causes

1. AQL query may have default row limit (e.g., 100 letters)
2. Letters created during page load not shown until manual refresh
3. No "refresh" button or auto-refresh mechanism in UI
