...
Multiple rapid API calls to `/patients` endpoint can cause race condition where older response overwrites newer data, or component re-renders trigger duplicate fetches with cleanup flag that doesn't prevent promise settlement.

---

## Causes

1. fetchPatients function does not cancel in-flight HTTP requests
2. Component re-renders trigger duplicate fetches
3. useEffect cleanup sets `cancelled = true` but fetch promise still settles after cleanup
