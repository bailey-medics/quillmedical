...
create_fhir_patient function does not check if patient with same NHS number or name/DOB combination already exists before creating new FHIR resource, allowing duplicate patient records.

---

## Causes

1. No search query before POST to FHIR server
2. FHIR server allows duplicate resources with different IDs
3. Frontend does not prevent duplicate submission
4. No uniqueness constraint on NHS number
