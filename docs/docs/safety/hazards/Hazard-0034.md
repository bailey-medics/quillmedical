...
API client infinite token refresh loop
...
API client retries request after 401 by refreshing token, but if refresh also fails with 401, potential for infinite redirect loop if login page also makes API calls that fail.

---

## Causes

1. Recursive call to request() with retry flag but no maximum retry count
2. Login page redirect may happen multiple times
3. No circuit breaker to stop retry attempts after certain threshold
