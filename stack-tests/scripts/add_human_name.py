#!/usr/bin/env python3
"""Add one human name to every file in `stack-tests/text-files/`.

The second of the test plan's scripts. It writes to *all* the files in the
folder, including the one phase 1 added, which is what makes this branch
depend on the one below it.

Usage: add_human_name.py [name]
...
def main(argv: list[str]) -> int:
    ...
    for path in files:
        name = chosen if chosen else name_for(path, HUMAN_NAMES)
        add_name(path, name)
